...
Deno.serve(async (req) => {
  ...
  // SuperAdmin only — check is_superadmin flag
  ...
  try {
    const { orgId, orgName, adminEmail, plan } = await req.json()
    ...
    // Cancel existing subscription at period end before creating new one
    ...
    const monthlyRate = PLAN_RATES[plan] ?? 0
    await fetch(`${supabaseUrl}/rest/v1/organizations?id=eq.${orgId}`, {
      method: 'PATCH',
      headers: { ...dbHeaders, 'Prefer': 'return=minimal' },
      body: JSON.stringify({
