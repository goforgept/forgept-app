    ...
    for (const proposal of proposals) {
      try {
        ...
        if (daysUntilClose > 14) {
          ...
          clientBody = `<p>Hi ${clientName},</p><p>I hope things are going well. I wanted to follow up on the proposal we sent over for <strong>${proposal.proposal_name}</strong>.</p><p>The total investment for this project comes to <strong>$${proposalValue}</strong>. We're excited about the opportunity to work with ${proposal.company} and would love to answer any questions you might have.</p><p>Is there a good time this week to connect and talk through the details?</p><p>Looking forward to hearing from you.</p>`
        } else if (daysUntilClose > 0) {
          clientSubject = `Quick check-in — ${proposal.proposal_name}`
          clientBody = `<p>Hi ${clientName},</p><p>Just wanted to check in on the proposal for <strong>${proposal.proposal_name}</strong>. We're getting close to the date we had discussed and I want to make sure we have everything lined up for you.</p><p>If you have any questions about the scope of work or pricing, I'm happy to jump on a quick call. We want to make sure this project gets started on the right foot.</p><p>Let me know if there is anything I can do to move things forward.</p>`
        } else {
          clientSubject = `Today's the day — ${proposal.proposal_name}`
          clientBody = `<p>Hi ${clientName},</p><p>Today is the date we had targeted to move forward on <strong>${proposal.proposal_name}</strong>. I wanted to reach out personally to see where things stand.</p><p>We have everything ready on our end and are excited to get started. If now is not the right time, no pressure at all — I just want to make sure we stay in touch.</p><p>Either way, I'd love to hear from you today.</p>`
        }

        const emailFooter = `<br/><p>Best regards,<br/><strong>${repName}</strong><br/>${companyName}<br/>${repEmail}</p><br/><p style="color:#aaa;font-size:11px;">This email was sent on behalf of ${repName} at ${companyName}.</p>`
        ...
        if (proposal.client_email) {
          const clientEmailRes = await fetch('https://api.brevo.com/v3/smtp/email', {
            ...
            body: JSON.stringify({
              ...
              htmlContent: `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;border:1px solid #e0e0e0;border-radius:8px;overflow:hidden;">${logoHeader}<div style="padding:28px;">${clientBody}${emailFooter}</div></div>`
            })
          })
