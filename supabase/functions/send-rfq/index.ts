...
Deno.serve(async (req) => {
  ...
  try {
    const { lineItemId, itemName, partNumber, quantity, unit, vendorEmail, vendorName, proposalName, repName, repEmail, company } = await req.json()
    ...
    const emailResponse = await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({
        ...
        subject: `RFQ: ${itemName} — ${proposalName}`,
        htmlContent: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2 style="color: #0F1C2E;">Request for Quote</h2>
            <p>Hi ${vendorName},</p>
            <p>${repName} from ${company} is requesting a quote for the following item:</p>
            <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
              ...
              <tr style="background: #f5f5f5;">
                ...
                <td style="padding: 10px;">${partNumber || '—'}</td>
                <td style="padding: 10px;">${quantity}</td>
                <td style="padding: 10px;">${unit}</td>
              </tr>
            </table>
            <p>Please reply to this email with your best pricing.</p>
            <p>Project: <strong>${proposalName}</strong></p>
            <br/>
    ...
    // Update line item pricing_status to RFQ Sent
