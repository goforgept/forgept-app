-- Allow dev and product_manager roles to manage roadmap items
...
CREATE POLICY "roadmap_rep_insert" ON roadmap_items FOR INSERT WITH CHECK (
  ...
  AND (
    status = 'backlog'
    OR org_id IN (SELECT org_id FROM profiles WHERE id = auth.uid() AND org_role IN ('admin', 'dev', 'product_manager'))
  )
);
