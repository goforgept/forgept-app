-- The 20260702 pathway migration replaced sync_drawing_to_bom but dropped the
-- labor sync block. Also, the original 20260622 labor sync read
-- dp.labor_hours_override (never populated) instead of dp.labor_overrides
-- (the jsonb per-role object the UI actually writes).
-- This migration combines the full pathway sync with a corrected labor sync.

-- Ensure labor_overrides column exists (may have been added directly to prod DB)
ALTER TABLE drawing_placements
  ADD COLUMN IF NOT EXISTS labor_overrides jsonb;
...
BEGIN
  ...
  IF v_labor_enabled THEN
    ...
    -- Uses dp.labor_overrides (jsonb keyed by role) for per-placement hour
    -- overrides set in PlacementPanel; falls back to hours_per_unit from
    -- designer_labor_defaults. LEFT JOIN on labor_rates so a row still appears
    -- even if the role name doesn't match (cost will be $0 until rate card is fixed).
    UPDATE proposals
