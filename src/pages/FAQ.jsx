...
const faqs = [
  {
    ...
    items: [
      ...
      { q: 'How do I add clients?', a: 'Go to the Clients page and click Add Client. Once added, you can click any client to view all their proposals and create new ones pre-filled with their details. From the client detail page you can also Archive a client (hides them from the list but keeps their history) or Delete them permanently.' }
    ]
  },
  {
    category: 'Proposals',
  ...
  },
  {
    ...
    items: [
      ...
      { q: 'What invoice statuses are available?', a: 'Invoices can be set to Draft, Sent, Partially Paid, Paid, or Overdue. Use the status filter at the top of the Invoices page to view invoices by status. Overdue status is applied automatically when the due date passes and a balance is still owed.' },
      { q: 'How do I get notified about overdue invoices?', a: 'ForgePt automatically marks invoices as Overdue once their due date passes with a balance remaining, and sends a bell notification to the rep and org admins. The notification includes the client name, invoice number, amount, and original due date with a direct link to the invoice.' },
      { q: 'Where do payment terms show on an invoice?', a: 'Payment terms are pulled from the client record (set under the client\'s Net Terms field) and displayed in the invoice detail header alongside the invoice date and due date. They also appear on the PDF invoice. If you are connected to QuickBooks, terms are matched by name and sent to QBO so the invoice shows the correct terms there too.' },
      { q: 'Can I send an invoice through QuickBooks?', a: 'Yes — if your organization is connected to QuickBooks Online, an invoice detail page will show a QuickBooks button. Check the Send checkbox to have QBO email the invoice to the client automatically. Once pushed, the invoice status updates to Sent in ForgePt. If the client pays in QBO, the status syncs back to Paid automatically via webhook.' }
    ]
  },
  {
    category: 'Jobs',
  ...
  },
  {
    ...
    items: [
      ...
      { q: 'What is the Recurring Items tab on the Contracts page?', a: 'The Recurring Items tab shows all BOM line items across your proposals that have been marked as recurring — such as monthly monitoring fees or software subscriptions. From this tab you can enable or disable auto-invoicing for each item and see the next scheduled invoice date. Items with auto-invoice on will be billed automatically on their renewal date.' }
    ]
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { q: 'What are compliance fields on a product?', a: 'If your organization has the Compliance Fields feature enabled, each product in the library can store Lead Time, Country of Origin (COO), and Berry Amendment compliance status. When a drawing is pushed to a BOM or a BOM sync runs, these values are automatically pulled from the product library into the BOM line items by part number — so you are not re-entering them on every job.' },
      { q: 'How do compliance columns appear on proposals?', a: 'Compliance columns (Lead Time, COO, Berry Amendment) are hidden by default on every proposal even when the feature is enabled org-wide. To show them on a specific proposal, open the proposal, click the Pricing Options button, and toggle Show Compliance Columns on. This controls both the BOM view and the PDF/DOCX exports for that proposal.' }
    ]
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { q: 'How do I set up two-factor authentication (2FA)?', a: 'Go to Settings → Security and click Enable Two-Factor Authentication. Scan the QR code with an authenticator app (such as Google Authenticator or Authy), then enter the 6-digit code shown in the app to verify and activate 2FA. Once enabled, you will be prompted for a code each time you log in. To turn it off, go back to Settings → Security and click Disable.' },
      { q: 'What is my support code and where do I find it?', a: 'Your support code is a unique 8-character code found in Settings → Support. Copy it and share it with the ForgePt support team when starting a support session — it lets us quickly look up your account without needing sensitive login details.' },
      { q: 'How do I get remote support from ForgePt?', a: 'Go to Settings → Support for step-by-step instructions. For desktop support, the team uses Google Remote Desktop — install the Chrome Remote Desktop extension from the Chrome Web Store and share the one-time access code with support. For iOS, you can share your screen by starting an iPhone screen recording and mirroring it to a Mac via AirPlay, then sharing that Mac\'s screen. Have your support code ready so the team can pull up your account before the session starts.' }
    ]
  },
  {
    ...
    items: [
      ...
      { q: 'How do I connect QuickBooks Online?', a: 'Go to Settings → Integrations and click Connect QuickBooks. You will be redirected to Intuit to authorize the connection. Once connected, ForgePt can push invoices to QBO, sync customer records both ways, and receive payment status updates automatically.' },
      { q: 'How do invoices sync to QuickBooks?', a: 'Open any invoice in ForgePt and click the QuickBooks button. Line items, tax, due date, customer, and payment terms are all sent to QBO. If the invoice was already pushed before, clicking Re-sync updates the existing QBO invoice instead of creating a duplicate. Check the Send checkbox to have QBO email the invoice to the client at the same time.' },
      { q: 'How do customers sync between ForgePt and QuickBooks?', a: 'The sync works both ways automatically. When you add or edit a client in ForgePt, they are pushed to QBO as a customer. When you add or update a customer in QBO, the webhook fires and creates or updates the matching ForgePt client within a minute or two. To do a one-time bulk import of all existing QBO customers, go to Settings → Integrations → QuickBooks → Import Customers.' },
      { q: 'Does payment status sync back from QuickBooks?', a: 'Yes — when an invoice is marked paid in QBO, a webhook fires and automatically updates the ForgePt invoice status to Paid and records the payment. If QBO emails the invoice, the status also updates to Sent. Webhooks must be registered in the Intuit developer portal for this to work.' },
  ...
  },
  {
    ...
    items: [
      ...
      { q: 'Can I view Sales KPIs for previous months or weeks?', a: 'Yes — on the Sales KPIs page, use the ‹ and › arrows next to the period label to navigate backward or forward through past weeks and months. The label updates to show the period you are viewing (for example "Last Month" or "Aug 2026"). Click the Weekly or Monthly button at any time to jump back to the current period.' },
    ]
  },
]
