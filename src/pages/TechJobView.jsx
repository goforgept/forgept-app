      ...
      <div className="flex-1 p-6 space-y-6">
        ...
        <div className="bg-fp-card rounded-xl p-6">
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <div className="flex items-center gap-4 mt-2 text-sm text-fp-muted">
                ...
                {job?.start_date && (
                  <span>📅 Start: <span className="text-fp-text">{new Date(job.start_date + 'T12:00:00').toLocaleDateString()}</span></span>
                )}
                {job?.end_date && (
                  <span>🏁 End: <span className="text-fp-text">{new Date(job.end_date + 'T12:00:00').toLocaleDateString()}</span></span>
                )}
              </div>
            </div>
          ...
          <div className="mt-5">
            <div className="flex justify-between items-center mb-1">
              <span className="text-fp-muted text-xs">Job progress — {completedCount} of {checklist.length} items complete</span>
          ...
          {mySchedules.length > 0 && (
            <div className="mt-4 pt-4 border-t border-fp-border">
              <p className="text-fp-muted text-xs font-semibold uppercase tracking-wide mb-2">My Upcoming Schedule</p>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-fp-card rounded-xl p-4">
            <p className="text-fp-muted text-xs mb-1">Hours Logged</p>
            <p className={`text-2xl font-bold ${laborOver ? 'text-red-400' : 'text-fp-text'}`}>{hoursLogged.toFixed(1)}</p>
            {estimatedHours > 0 && (
              <p className="text-fp-muted text-xs mt-1">of {estimatedHours.toFixed(1)} estimated</p>
            )}
          </div>
          <div className="bg-fp-card rounded-xl p-4">
            <p className="text-fp-muted text-xs mb-1">Checklist</p>
            <p className="text-fp-text text-2xl font-bold">{completedCount}/{checklist.length}</p>
            <p className="text-fp-muted text-xs mt-1">{progress}% complete</p>
          </div>
          <div className="bg-fp-card rounded-xl p-4">
            <p className="text-fp-muted text-xs mb-1">Crew on Job</p>
            <p className="text-fp-text text-2xl font-bold">{[...new Set(jobSchedules.map(s => s.tech_id))].length}</p>
            <p className="text-fp-muted text-xs mt-1">team members scheduled</p>
          </div>
        </div>
        ...
        {activeTab === 'overview' && (
          <div className="space-y-4">
            ...
            {job?.scope_of_work && (
              <div className="bg-fp-card rounded-xl p-6">
                <h3 className="text-fp-text font-bold text-lg mb-3">📋 PM Notes / Scope of Work</h3>
                <p className="text-fp-text text-sm leading-relaxed whitespace-pre-wrap">
            ...
            {/* Labor hours summary */}
            <div className="bg-fp-card rounded-xl p-6">
              <h3 className="text-fp-text font-bold text-lg mb-4">⏱ Hours Status</h3>
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-fp-muted text-sm">Hours logged</span>
                  <span className="text-fp-text font-bold">{hoursLogged.toFixed(1)} hrs</span>
                </div>
                {estimatedHours > 0 && (
                  <>
                    <div className="flex justify-between items-center">
                      <span className="text-fp-muted text-sm">Estimated total</span>
                      <span className="text-fp-text">{estimatedHours.toFixed(1)} hrs</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-fp-muted text-sm">Remaining</span>
            ...
            {techLogs.length > 0 && (
              <div className="bg-fp-card rounded-xl p-6">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-fp-text font-bold text-lg">Recent Work Logs</h3>
                  <button onClick={() => navigate('/tech-log')} className="text-[#C8622A] text-sm hover:text-fp-text transition-colors">View All →</button>
                </div>
                <div className="space-y-3">
                  {techLogs.slice(0, 3).map(log => (
                    <div key={log.id} className="bg-fp-inset rounded-lg p-4">
                      <div className="flex justify-between items-start mb-1">
                        <div className="flex items-center gap-2">
                          <span className="text-fp-muted text-xs font-mono">
        ...
        {activeTab === 'checklist' && (
          <div className="bg-fp-card rounded-xl p-6">
            ...
            {checklist.length === 0 ? (
              <p className="text-fp-muted text-sm">No checklist items yet.</p>
            ) : (
              <div className="space-y-2">
                {checklist.map(item => (
                  <div key={item.id} className={`flex items-center gap-3 p-3 rounded-lg border ${
                    item.completed ? 'bg-green-500/5 border-green-500/20' : 'bg-fp-inset border-fp-border'
                  }`}>
                    <div className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 ${
                      item.completed ? 'bg-green-500 border-green-500' : 'border-fp-border'
                    }`}>
                      {item.completed && <span className="text-fp-text text-xs">✓</span>}
                    </div>
                    <div className="flex-1">
                      <p className={`text-sm font-medium ${item.completed ? 'text-green-400 line-through' : 'text-fp-text'}`}>
                        {item.label}
                      </p>
                    ...
                    {item.is_auto && (
                      <span className="text-xs px-2 py-0.5 rounded-full text-fp-muted bg-fp-inset">Auto</span>
                    )}
                  </div>
                ))}
              </div>
        ...
        {activeTab === 'materials' && (
          <div className="bg-fp-card rounded-xl p-6">
            <h3 className="text-fp-text font-bold text-lg mb-5">Materials</h3>
            {lineItems.length === 0 ? (
              <p className="text-fp-muted text-sm">No materials on this job.</p>
            ) : (
              <div className="space-y-2">
                {lineItems.map(item => {
                  ...
                  return (
                    <div key={item.id} className={`bg-fp-inset rounded-xl p-4 border ${
                      isOver ? 'border-red-500/30' : isLow ? 'border-yellow-500/30' : 'border-fp-border'
                    }`}>
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <div className="text-right text-xs">
                          <p className="text-fp-muted">Planned: <span className="text-fp-text font-semibold">{planned} {item.unit}</span></p>
        ...
        {activeTab === 'hours' && (
          <div className="bg-fp-card rounded-xl p-6">
            <div className="flex justify-between items-center mb-5">
              <h3 className="text-fp-text font-bold text-lg">Hours Logged</h3>
              <button onClick={() => navigate('/tech-log')}
                className="bg-fp-brand text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-[#b5571f] transition-colors">
                + Log Work
              </button>
            ...
            {laborItems.length > 0 && laborItems.some(l => l.role) && (
              <div className="bg-fp-inset rounded-xl p-4 mb-4">
                <p className="text-fp-muted text-xs font-semibold uppercase tracking-wide mb-3">Planned Labor</p>
                <div className="space-y-2">
            ...
            {estimatedHours > 0 && (
              <div className="bg-fp-inset rounded-xl p-4 mb-4">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-fp-muted text-sm">Progress</span>
                  <span className={`font-bold ${laborOver ? 'text-red-400' : 'text-fp-text'}`}>
                    {hoursLogged.toFixed(1)} / {estimatedHours.toFixed(1)} hrs
                  </span>
                </div>
                <div className="w-full bg-fp-card rounded-full h-2 mb-2">
                  <div className={`h-2 rounded-full ${laborOver ? 'bg-red-500' : 'bg-blue-500'}`}
                    style={{ width: `${laborPct}%` }} />
                </div>
            ...
            {techLogs.length === 0 ? (
              <div className="text-center py-8 border-2 border-dashed border-fp-border rounded-xl">
                <p className="text-fp-muted mb-3">No hours logged yet.</p>
                <button onClick={() => navigate('/tech-log')}
                  className="text-[#C8622A] hover:text-fp-text text-sm transition-colors">+ Log Today's Work →</button>
              </div>
            ) : (
              <div className="space-y-3">
                {techLogs.map(log => (
        ...
        {activeTab === 'crew' && (
          <div className="bg-fp-card rounded-xl p-6">
            <h3 className="text-fp-text font-bold text-lg mb-5">Crew & Schedule</h3>

            {/* All upcoming schedules */}
            {allUpcoming.length === 0 ? (
              <p className="text-fp-muted text-sm">No upcoming schedules.</p>
            ) : (
              <div className="space-y-2">
                {allUpcoming.map(s => (
                  <div key={s.id} className={`flex items-center justify-between p-3 rounded-lg border ${
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className={`text-sm font-medium ${s.tech_id === profile?.id ? 'text-[#C8622A]' : 'text-fp-text'}`}>
            ...
            {jobSchedules.length > 0 && (
              <div className="mt-6">
                <p className="text-fp-muted text-xs font-semibold uppercase tracking-wide mb-3">Team on This Job</p>
                <div className="flex flex-wrap gap-2">
