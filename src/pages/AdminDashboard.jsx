        ...
        <div className="bg-[#1a2d45] rounded-xl p-6">
          <div className="flex justify-between items-center mb-5">
            <div>
              ...
              <p className="text-[#8A9AB0] text-xs mt-0.5">Use this to plan crew scheduling and backlog</p>
            </div>
            {laborStats.dealsWithLabor > 0 && (
              <span className="text-[#8A9AB0] text-xs">{laborStats.dealsWithLabor} deal{laborStats.dealsWithLabor !== 1 ? 's' : ''} with labor</span>
            )}
          </div>
          {loading ? (
            <p className="text-[#8A9AB0]">Loading...</p>
          ) : laborStats.laborQuoted === 0 && laborStats.laborWon === 0 ? (
            <p className="text-[#8A9AB0] text-sm">No labor data yet. Add labor items to proposals to see your forecast.</p>
          ) : (
            <div className="grid grid-cols-4 gap-4">
              ...
              <div className="bg-[#0F1C2E] rounded-xl p-4">
                ...
                <p className="text-white text-xl font-bold">${laborStats.avgLaborPerDeal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                <p className="text-[#8A9AB0] text-xs mt-1">Across active deals</p>
              </div>
            </div>
          )}
        </div>
