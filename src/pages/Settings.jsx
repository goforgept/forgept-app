...
export default function Settings({ isAdmin, featureProposals = true, featureCRM = false, featurePurchaseOrders = true, featureInvoices = true, featureSla = false, featureMonitoring = false, featureDesignerOnly = false, role, isSalesManager, isPM, isTechnician }) {
  ...
  const [orgServiceSettings, setOrgServiceSettings] = useState({
    trip_fee_default: '',
    drive_time_rate_default: '',
    service_billing_mode: 'trip_fee'
  })
  ...
  const fetchProfile = async () => {
    ...
    // Fetch labor rates and org service settings
    if (data?.org_id) {
      ...
      const { data: orgData } = await supabase
        .from('organizations')
        .select('trip_fee_default, drive_time_rate_default, service_billing_mode')
        .eq('id', data.org_id)
        .single()
      if (orgData) {
        setOrgServiceSettings({
          trip_fee_default: orgData.trip_fee_default || '',
          drive_time_rate_default: orgData.drive_time_rate_default || '',
          service_billing_mode: orgData.service_billing_mode || 'trip_fee'
        })
      }
    }
  ...
  const handleChangePassword = async () => {
    ...
    if (signInError) { setPasswordError('Current password is incorrect'); setSavingPassword(false); return }
  ...
  const saveRateCard = async () => {
    setSavingRates(true)
    try {
      // Save labor rates — delete all and reinsert
      ...
      if (laborRates.length > 0) {
        await supabase.from('labor_rates').insert(
          laborRates.map((r, i) => ({
            ...
            bill_rate_per_hour: parseFloat(r.bill_rate_per_hour) || 0,
            unit: r.unit || 'hr',
            sort_order: i
          }))
        )
      }
      // Save org service billing settings
      ...
      <div className="flex-1 p-6 max-w-4xl">
        ...
        <div className="flex gap-1.5 mb-6 overflow-x-auto pb-1 scrollbar-none border-b border-[#2a3d55]">
          {[
            ...
            ...(isAdmin && !featureDesignerOnly ? [{ key: 'integrations', label: 'Integrations' }] : []),
            ...(isAdmin && !featureDesignerOnly ? [{ key: 'email',        label: 'Email Templates' }] : []),
            ...(isAdmin && !featureDesignerOnly ? [{ key: 'invoicing',    label: 'Invoicing' }] : []),
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-end gap-3">
              <button onClick={async () => {
                // Also save default markup when saving rate card
                await supabase.from('profiles').update({
                  default_markup_percent: parseFloat(form.default_markup_percent) || 35
                }).eq('id', profile.id)
                await saveRateCard()
...
{activeTab === 'designer' && isAdmin && (
          <div className="space-y-8">
            ...
            <div className="bg-[#1a2d45] border border-[#2a3d55] rounded-xl p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ['Designer / Engineer Name', 'title_block_engineer', 'text'],
                  ['License Number', 'title_block_license', 'text'],
                  ['Default Scale', 'title_block_scale', 'text'],
            ...
            <div className="bg-[#1a2d45] border border-[#2a3d55] rounded-xl p-6">
              ...
              <p className="text-[#8A9AB0] text-xs mb-4">
                Global symbols are managed by ForgePt and available to all orgs.
                Add your own custom symbols for devices not in the global library.
              </p>
              ...
              <div className="bg-[#0F1C2E] rounded-lg border border-[#2a3d55] p-8 text-center">
                ...
                <p className="text-[#8A9AB0] text-xs mt-1">
                  Add devices not in the global library — your part numbers, descriptions, and category icons
                </p>
              </div>
            </div>
            ...
            <div className="bg-[#1a2d45] border border-[#2a3d55] rounded-xl p-6">
              ...
              <p className="text-[#8A9AB0] text-xs mb-4">
                Save device configurations as templates to apply quickly during design.
              </p>
              <div className="bg-[#0F1C2E] rounded-lg border border-[#2a3d55] p-8 text-center">
                <p className="text-[#8A9AB0] text-sm">No templates yet</p>
                <p className="text-[#8A9AB0] text-xs mt-1">
                  Templates are created from the Designer when you save a device configuration
                </p>
              </div>
            </div>
            ...
            <div className="bg-[#1a2d45] border border-[#2a3d55] rounded-xl p-6">
              ...
              <p className="text-[#8A9AB0] text-xs mb-4">
                Control when inactive projects are flagged for cleanup.
                You will always be prompted before anything is deleted.
              </p>
              <div className="grid grid-cols-2 gap-4">
                <div>
        ...
        {activeTab === 'email' && isAdmin && (
          <div className="space-y-6">
            ...
            <button onClick={handleSaveTemplates} disabled={savingTemplates} className="bg-[#C8622A] text-white px-6 py-3 rounded-lg font-semibold hover:bg-[#b5571f] transition-colors disabled:opacity-50">
              ...
              {/* ── TEAM TAB ── */}
        {activeTab === 'team' && (
          <TeamSettingsTab featureDesignerOnly={featureDesignerOnly} />
        )}
...
// ─── TeamSettingsTab ──────────────────────────────────────────────────────────
function TeamSettingsTab({ featureDesignerOnly }) {
  const [members,  setMembers]  = useState([])
  const [loading,  setLoading]  = useState(true)
  const [showForm, setShowForm] = useState(false)
  const [form,     setForm]     = useState({ email: '', full_name: '', org_role: featureDesignerOnly ? 'rep' : 'rep' })
