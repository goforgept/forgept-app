...
export default function Settings({ isAdmin, featureProposals = true, featureCRM = false, featurePurchaseOrders = true, featureInvoices = true, featureSla = false, featureMonitoring = false, featureDesignerOnly = false, featureApi = false, role, isSalesManager, isPM, isTechnician, isDevTeam = false }) {
  ...
  const fetchProfile = async () => {
    ...
    if (data?.org_id) {
      try {
        ...
        // Migrate from old single warranty_text field if templates not yet set
        const existingTemplates = orgData?.warranty_templates
        if (existingTemplates?.length) {
          setWarrantyTemplates(existingTemplates)
        } else if (orgData?.warranty_text) {
          setWarrantyTemplates([{ id: crypto.randomUUID(), name: 'Warranty', text: orgData.warranty_text }])
        } else {
  ...
  const handleSave = async () => {
    ...
    if (orgId) await supabase.from('organizations').update({ timezone: orgTimezone, default_hide_material_prices: defaultHideMaterialPrices, default_hide_labor_breakdown: defaultHideLaborBreakdown, default_lump_sum_labor: defaultLumpSumLabor, warranty_templates: warrantyTemplates.filter(t => t.name || t.text) }).eq('id', orgId)
    refreshProfile()
        ...
        <div className="flex-1 p-6 min-w-0 max-w-3xl">
          ...
          {activeTab === 'proposals' && (
            <ProposalsTab form={form} setForm={setForm} inputClass={inputClass}
              ...
              warrantyTemplates={warrantyTemplates} setWarrantyTemplates={setWarrantyTemplates}
              ...
              onChangePdfHeaderStyle={async (val) => {
