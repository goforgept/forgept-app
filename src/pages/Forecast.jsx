...
export default function Forecast({ isAdmin }) {
  ...
  const fmtK = (num) => num >= 1000 ? `$${(num / 1000).toFixed(1)}k` : `$${fmt(num)}`
  ...
  const monthlyWon = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const won = proposals
        .filter(p => p.status === 'Won' && p.created_at?.startsWith(key))
  ...
  // Weighted pipeline — probability by stage position
  ...
  const weightedPipeline = useMemo(() => {
    return stageBreakdown.reduce((sum, stage) => {
      const prob = stageProbability(stage.name) / 100
      return sum + (stage.value * prob)
    }, 0)
  }, [stageBreakdown])
  ...
  const repForecast = useMemo(() => {
    ...
    proposals.filter(p => p.status !== 'Lost').forEach(p => {
      ...
      if (!repMap[rep]) repMap[rep] = { name: rep, pipeline: 0, won: 0, weighted: 0, count: 0 }
  ...
  const totalActivePipeline = proposals
    .filter(p => p.status !== 'Won' && p.status !== 'Lost')
    .reduce((sum, p) => sum + (p.proposal_value || 0), 0)

  const totalWon = proposals
              ...
              {monthlyWon.map(m => (
                <div key={m.key}>
                  ...
                  <div className="h-6 bg-[#0F1C2E] rounded-lg overflow-hidden relative">
                    {/* Total bar */}
                    <div
                      className="absolute inset-y-0 left-0 bg-[#2a3d55] rounded-lg transition-all duration-500"
                      style={{ width: `${(m.total / maxMonthlyValue) * 100}%` }}
                    />
                    {/* Won bar */}
                    <div
                      className="absolute inset-y-0 left-0 bg-green-500/70 rounded-lg transition-all duration-500"
            ...
            <div className="space-y-3">
              {closeRateByMonth.map(m => (
                <div key={m.key}>
                  ...
                  <div className="h-5 bg-[#0F1C2E] rounded-lg overflow-hidden">
                    <div
                      className="h-full bg-[#C8622A]/70 rounded-lg transition-all duration-500"
                      style={{ width: `${m.rate}%` }}
                    />
                  </div>
                </div>
        ...
        {closingThisMonth.length > 0 && (
          <div className="bg-[#1a2d45] rounded-xl p-6">
            <h3 className="text-white font-bold text-lg mb-4">
              Closing This Month
              <span className="ml-2 text-sm font-normal text-[#8A9AB0]">
                — ${fmt(closingThisMonth.reduce((s, p) => s + (p.proposal_value || 0), 0))} at stake
              </span>
            </h3>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <span className={`px-3 py-1 rounded-full text-xs font-semibold bg-[#8A9AB0]/20 text-[#8A9AB0]`}>
                        {p.status}
                      </span>
                    </div>
                  </div>
