...
export default function Jobs({ isAdmin, featureProposals = true, featureCRM = false, featurePurchaseOrders = true, featureInvoices = true }) {
  ...
  const fetchJobs = async () => {
    ...
    const { data } = await supabase
      .from('jobs')
      .select('*, proposals(proposal_name, proposal_value), clients(company), pm:profiles!jobs_user_id_fkey(full_name), tech:profiles!jobs_tech_id_fkey(full_name), job_checklist_items(id, completed)')
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 text-sm text-[#8A9AB0]">
                        ...
                        {job.pm?.full_name && <span>👤 PM: {job.pm.full_name}</span>}
                        {job.tech?.full_name && <span>🔧 {job.tech.full_name}</span>}
