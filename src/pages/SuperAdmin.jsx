...
// ─── AccessoriesEditor ────────────────────────────────────────────────────────
function AccessoriesEditor({ product, onClose, onSaved }) {
  ...
  const addRequired = () => {
    ...
    setAccessories(prev => ({
      ...prev,
      required: [...(prev.required || []), { ...reqForm, quantity: parseInt(reqForm.quantity) || 1 }]
    }))
    setReqForm({ type: '', part_number: '', name: '', manufacturer: product.manufacturer || '', quantity: 1 })
  }

  const removeRequired = (idx) => {
    setAccessories(prev => ({ ...prev, required: prev.required.filter((_, i) => i !== idx) }))
  }
  ...
  const addOptionGroup = () => {
    ...
    setAccessories(prev => ({
      ...prev,
      options: [...(prev.options || []), { ...optForm, choices: [] }]
    }))
    setOptForm({ group: '', required: true, default: '' })
  }

  const removeOptionGroup = (idx) => {
    setAccessories(prev => ({ ...prev, options: prev.options.filter((_, i) => i !== idx) }))
    if (editingGroupIdx === idx) setEditingGroupIdx(null)
  }
  ...
  const addChoice = (groupIdx) => {
    ...
    setAccessories(prev => {
      ...
      options[groupIdx] = {
        ...options[groupIdx],
        choices: [...(options[groupIdx].choices || []), { ...optChoice }]
      }
      return { ...prev, options }
    })
  ...
  const removeChoice = (groupIdx, choiceIdx) => {
    setAccessories(prev => {
      const options = [...prev.options]
      options[groupIdx].choices = options[groupIdx].choices.filter((_, i) => i !== choiceIdx)
      return { ...prev, options }
    })
  }
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
              ...
              <div className="bg-[#0F1C2E] rounded-xl border border-[#2a3d55] divide-y divide-[#2a3d55]/50 mb-3">
                {accessories.required.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-3 px-3 py-2">
                    ...
                    <button onClick={() => removeRequired(idx)} className="text-red-400 hover:text-red-300 text-xs">✕</button>
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="grid grid-cols-5 gap-2">
              <input placeholder="Type (e.g. Lock)" value={reqForm.type}
                onChange={e => setReqForm(p => ({ ...p, type: e.target.value }))}
                className={inputClass} />
              <input placeholder="Part #" value={reqForm.part_number}
                onChange={e => setReqForm(p => ({ ...p, part_number: e.target.value }))}
                className={inputClass} />
              <input placeholder="Name" value={reqForm.name}
                onChange={e => setReqForm(p => ({ ...p, name: e.target.value }))}
                className={inputClass} />
              <input placeholder="Mfr" value={reqForm.manufacturer}
                onChange={e => setReqForm(p => ({ ...p, manufacturer: e.target.value }))}
                className={inputClass} />
              <div className="flex gap-1">
                <input type="number" min="1" placeholder="Qty" value={reqForm.quantity}
                  onChange={e => setReqForm(p => ({ ...p, quantity: e.target.value }))}
                  className={`${inputClass} w-12`} />
                <button onClick={addRequired}
                  className="bg-[#C8622A] text-white px-2 py-1 rounded text-xs hover:bg-[#b5571f] transition-colors">
                  Add
                </button>
              </div>
            </div>
          ...
          <div>
              ...
              <div key={groupIdx} className="bg-[#0F1C2E] rounded-xl border border-[#2a3d55] mb-3">
                <div className="flex items-center justify-between px-3 py-2 border-b border-[#2a3d55]">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => removeOptionGroup(groupIdx)}
                ...
                {(group.choices || []).map((choice, choiceIdx) => (
                  <div key={choiceIdx} className="flex items-center gap-3 px-3 py-1.5 border-b border-[#2a3d55]/30">
                    ...
                    {group.default === choice.part_number && (
                      <span className="text-xs bg-green-900/40 text-green-400 px-1.5 py-0.5 rounded">Default</span>
                    )}
                    <button onClick={() => {
                      setAccessories(prev => {
                        const options = [...prev.options]
                        options[groupIdx] = { ...options[groupIdx], default: choice.part_number }
                        return { ...prev, options }
                      })
                    }} className="text-xs text-[#8A9AB0] hover:text-green-400 transition-colors">
                      Set Default
                    </button>
                    <button onClick={() => removeChoice(groupIdx, choiceIdx)}
                      className="text-red-400 hover:text-red-300 text-xs">✕</button>
                  </div>
                ))}
            ...
            <div className="flex gap-2">
              <input placeholder="Group name (e.g. Mount Type)" value={optForm.group}
                onChange={e => setOptForm(p => ({ ...p, group: e.target.value }))}
                className={`${inputClass} flex-1`} />
              <label className="flex items-center gap-1.5 text-xs text-[#8A9AB0] cursor-pointer">
                <input type="checkbox" checked={optForm.required}
                  onChange={e => setOptForm(p => ({ ...p, required: e.target.checked }))}
                  className="accent-[#C8622A]" />
                Required
              </label>
              <button onClick={addOptionGroup}
                className="bg-[#2a3d55] text-white px-3 py-1.5 rounded text-xs hover:bg-[#3a4d65] transition-colors flex-shrink-0">
                Add Group
              </button>
            </div>
          </div>
