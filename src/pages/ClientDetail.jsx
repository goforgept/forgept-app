    ...
    <div className="flex min-h-screen bg-fp-inset">
      ...
      <div className="flex-1 p-6 space-y-6 min-w-0">
        ...
        <div className="bg-fp-card rounded-xl px-6 py-4 flex gap-8 flex-wrap">
          ...
          {client?.website && <div className="flex items-center gap-2"><span className="text-fp-muted text-xs uppercase tracking-wide">Website</span><button onClick={() => window.open(/^https?:\/\//i.test(client.website) ? client.website : `https://${client.website}`, '_blank')} className="text-[#C8622A] text-sm hover:underline text-left">{client.website.replace(/^https?:\/\//i, '')}</button></div>}
...
{activeTab === 'meetings' && (
        ...
        {activeTab === 'activity' && <ActivityTimeline clientId={id} orgId={client?.org_id} userId={profile?.id} contacts={contacts} proposals={proposals} />}
