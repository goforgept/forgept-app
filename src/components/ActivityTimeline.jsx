  ...
  return (
    <div className="bg-fp-card rounded-xl p-6">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-fp-text font-bold text-lg">Activity</h3>
        <button
            ...
            {ACTIVITY_TYPES.map(t => (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                  form.type === t.value ? 'bg-fp-brand text-white' : 'bg-fp-card text-fp-muted hover:text-fp-text'
                }`}
              >
                {t.icon} {t.label}
              </button>
      ...
      {loading ? (
        ...
        <p className="text-fp-muted text-sm">No activity yet. Log a call, email, or note above.</p>
      ) : (
        <div className="space-y-0">
          {activities.map((activity, i) => (
            <div key={activity.id} className="flex gap-3 relative">
              {i < activities.length - 1 && (
                <div className="absolute left-4 top-8 bottom-0 w-px bg-fp-inset" />
              )}
              <div className="w-8 h-8 rounded-full bg-fp-inset border border-fp-border flex items-center justify-center text-sm shrink-0 z-10">
                {getIcon(activity.type)}
              </div>
              <div className="flex-1 pb-4">
                <div className="flex justify-between items-start">
                  <p className="text-fp-text text-sm font-medium">{activity.title}</p>
                  <span className="text-fp-muted text-xs shrink-0 ml-2">{formatDate(activity.created_at)}</span>
                </div>
                {activity.body && (
                  <p className="text-fp-muted text-xs mt-1 leading-relaxed">{activity.body}</p>
                )}
                <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                  <p className="text-fp-muted text-xs">{activity.profiles?.full_name}</p>
                  {activity.client_contacts?.full_name && (
                    <span className="text-fp-muted text-xs">· with {activity.client_contacts.full_name}</span>
                  )}
                  {activity.proposals?.proposal_name && (
                    <span className="text-fp-muted text-xs">· {activity.proposals.proposal_name}</span>
                  )}
                </div>
              </div>
            </div>
