  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-4">
      <div className="bg-fp-card rounded-2xl p-6 w-full max-w-md">
        ...
        <h3 className="text-fp-text font-bold text-lg mb-1 text-center">Set Contract Start Dates</h3>
        <p className="text-fp-muted text-sm mb-5 text-center">Set a start date for each agreement. End date will be auto-set to 1 year later.</p>
        <div className="space-y-3 mb-5">
          {pendingContractItems.map(item => (
            <div key={`${item._type}_${item._idx}`} className="bg-fp-inset rounded-lg px-4 py-3">
              <div className="flex justify-between items-center gap-3">
                <div>
                  <p className="text-fp-text text-sm font-medium">{item.name || (item._type === 'sla' ? 'Service Agreement' : 'Monitoring Contract')}</p>
                  <p className="text-fp-muted text-xs capitalize">{item._type}</p>
                </div>
        ...
        <div className="flex gap-3">
          ...
          <button onClick={onSave} disabled={savingContractDates} className="flex-1 bg-fp-brand text-white py-2 rounded-lg text-sm font-semibold hover:bg-[#b5571f] transition-colors disabled:opacity-50">
