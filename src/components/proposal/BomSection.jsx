    ...
    <div className="bg-fp-card rounded-xl p-6">
      {canEdit && <div className="mb-4 relative">
        <input
          ...
          className="w-full bg-fp-inset text-fp-text border border-fp-border rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-fp-brand placeholder-[#8A9AB0]"
        />
        {librarySearch && (
          <button onClick={() => { setLibrarySearch(''); setLibraryResults([]); setShowLibrarySearch(false) }}
            className="absolute right-3 top-2.5 text-fp-muted hover:text-fp-text text-lg leading-none">×</button>
        )}
        {showLibrarySearch && (libraryLoading || libraryResults.length > 0) && (
          <div className="absolute top-full left-0 right-0 z-40 bg-fp-card border border-fp-border rounded-xl mt-1 max-h-96 overflow-y-auto shadow-2xl">
            {libraryLoading ? (
              <p className="text-fp-muted text-sm p-4">Searching...</p>
            ) : (
              <>
                <div className="p-3 space-y-1">
                  {libraryResults.map(prod => {
                    ...
                    return (
                      <div key={prod.id}
                        className={`rounded-lg p-3 border transition-colors cursor-pointer ${isChecked ? 'border-[#C8622A] bg-[#C8622A]/5' : 'border-fp-border hover:border-[#3a4d65]'}`}
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex items-start gap-3 min-w-0">
                            <input type="checkbox" checked={isChecked} readOnly className="accent-fp-brand mt-0.5 shrink-0" />
                            <div className="min-w-0">
                              <p className="text-fp-text text-sm font-semibold truncate">{prod.item_name}</p>
                              <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                                {prod.manufacturer && <span className="text-fp-muted text-xs">{prod.manufacturer}</span>}
                                {prod.part_number && <span className="text-fp-muted text-xs font-mono bg-fp-inset px-1.5 py-0.5 rounded">{prod.part_number}</span>}
                                {prod.category && <span className="text-fp-muted text-xs">{prod.category}</span>}
                          ...
                          <div className="shrink-0 text-right" onClick={e => e.stopPropagation()}>
                            {prod._fromCatalog ? (
                              <div>
                                {prod.msrp && <p className="text-fp-muted text-xs">MSRP ${Number(prod.msrp).toFixed(2)}</p>}
                              ...
                              <div>
                                ...
                                <p className="text-fp-muted text-xs">{prices[0].vendor}</p>
                              ...
                              <div>
                                <select
                                  ...
                                  className="bg-fp-inset text-fp-text border border-fp-border rounded px-2 py-1 text-xs focus:outline-none focus:border-fp-brand mb-1"
                                >
                ...
                {librarySelectedItems.size > 0 && (
                  <div className="border-t border-fp-border px-4 py-3 flex items-center justify-between sticky bottom-0 bg-fp-card">
                    <p className="text-fp-muted text-xs">{librarySelectedItems.size} item{librarySelectedItems.size !== 1 ? 's' : ''} selected</p>
                    <button onClick={onAddLibraryItems} className="bg-fp-brand text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-[#b5571f] transition-colors">Add to BOM →</button>
                  </div>
                )}
              </>
            )}
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        {canEdit && (!editingBOM ? (
          ...
          <div className="flex gap-2">
            <button onClick={onCancelEditing} className="px-4 py-2 text-fp-muted hover:text-fp-text text-sm transition-colors">Cancel</button>
      ...
      {rfqRequests.length > 0 && (
        <div className="bg-fp-card rounded-xl p-5 mb-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-fp-text font-bold">RFQ Status</h3>
            <span className="text-fp-muted text-xs">{rfqRequests.filter(r => r.status === 'responded').length} of {rfqRequests.length} responded</span>
          </div>
            ...
            {rfqRequests.map(rfq => {
              ...
              return (
                <div key={rfq.id} className={`rounded-xl p-4 border ${
                  ...
                  'bg-fp-inset border-fp-border'
                }`}>
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <p className="text-fp-text font-semibold text-sm">{rfq.vendor_name}</p>
                      ...
                      <p className="text-fp-muted text-xs">{itemCount} item{itemCount !== 1 ? 's' : ''} · Sent {new Date(rfq.sent_at || rfq.created_at).toLocaleDateString()}</p>
                      ...
                      {isResponded && rfq.vendor_quote_expiry && (
                        <p className="text-fp-muted text-xs">Expires {new Date(rfq.vendor_quote_expiry).toLocaleDateString()}</p>
                      )}
                      {isResponded && rfq.vendor_notes && (
                        <p className="text-fp-muted text-xs mt-1 italic">"{rfq.vendor_notes}"</p>
                      )}
                    </div>
                    <div className="flex flex-col items-end gap-2">
                      {!isResponded && rfq.expires_at && (
                        <p className={`text-xs ${isExpired ? 'text-red-400' : 'text-fp-muted'}`}>
      ...
      {!editingBOM ? (
        lineItems.length === 0 ? (
          <p className="text-fp-muted">No line items yet. Click Edit BOM to add items.</p>
        ) : (
                            ...
                            <td className="py-3">
                              <div className="flex flex-col gap-1">
                                ...
                                {item.rfq_expires_at && item.pricing_status === 'RFQ Sent' && (() => {
                                  ...
                                  return expired ? <span className="text-xs font-semibold px-2 py-1 rounded bg-red-500/20 text-red-400">⚠ Pricing Expired</span> : <span className="text-xs px-2 py-0.5 rounded bg-fp-inset text-fp-muted">Exp {new Date(item.rfq_expires_at).toLocaleDateString()}</span>
                                })()}
                              </div>
                            </td>
                            <td className="py-3 pr-2 text-center">
                              <div className="flex flex-col items-center gap-1">
                                <input type="checkbox" checked={!!item.recurring} onChange={() => onToggleRecurring(item.id, !!item.recurring)} className="accent-fp-brand cursor-pointer" title="Mark as recurring" />
                                {item.recurring && proposal?.status === 'Won' && (
                                  <input type="date" value={renewalDates[item.id] || item.renewal_date || ''} onChange={e => onSaveRenewalDate(item.id, e.target.value)}
                                    className="bg-fp-inset text-[#C8622A] border border-[#C8622A]/40 rounded px-1 py-0.5 text-xs focus:outline-none focus:border-fp-brand w-28" />
                                )}
                              </div>
                            </td>
                          </tr>
              ...
              return (
                <>
                  {unsectioned.length > 0 && (
                    <div>
                      {sections.length > 0 && (
                        <p className="text-fp-muted text-xs font-semibold uppercase tracking-wide mb-2">General</p>
                      )}
                  ...
                  {sections.map(section => {
                    ...
                    return (
                      <div key={section.id} className="border border-fp-border rounded-xl overflow-hidden">
                        <div className="flex items-center justify-between px-4 py-3 bg-fp-inset border-b border-fp-border">
                          ...
                          <span className="text-fp-muted text-xs">Section Total: <span className="text-fp-text font-bold">${secTotal.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span></span>
                        </div>
                        <div className="p-4">
                          ...
                          {section.include_labor && (section.labor_items || []).filter(l => l.role).length > 0 && (
                            <div className="mt-4 pt-4 border-t border-fp-border">
                              ...
                              <table className="w-full text-sm">
                                <thead>
                                  <tr className="border-b border-fp-border">
                                    {['Role', 'Qty', 'Unit', 'Total'].map(h => <th key={h} className="text-fp-muted text-left py-2 pr-4 font-normal text-xs">{h}</th>)}
                                  </tr>
                                </thead>
                                <tbody>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            {canEdit && (
              <button onClick={onAddSection} className="bg-fp-inset text-fp-text px-3 py-1.5 rounded-lg text-xs font-semibold hover:bg-fp-hover transition-colors">
                + Add Section
              </button>
            )}
          </div>
          ...
          {/* Bulk Edit Bar */}
          <div className="flex items-center gap-2 mb-4 p-3 bg-fp-inset rounded-lg border border-fp-border flex-wrap">
            <span className="text-fp-muted text-xs font-semibold whitespace-nowrap">
              Bulk Edit {bulkSelectedLines.size > 0 ? <span className="text-[#C8622A]">({bulkSelectedLines.size} selected)</span> : <span className="text-fp-muted">(check rows below)</span>}
            </span>
            <select value={bulkField} onChange={e => { setBulkField(e.target.value); setBulkValue('') }}
              className="bg-fp-card text-fp-text border border-fp-border rounded px-2 py-1.5 text-xs focus:outline-none focus:border-fp-brand">
            ...
            {bulkField === 'category' ? (
              <select value={bulkValue} onChange={e => setBulkValue(e.target.value)} className="bg-fp-card text-fp-text border border-fp-border rounded px-2 py-1.5 text-xs focus:outline-none focus:border-fp-brand">
              ...
              <input type={bulkField === 'markup_percent' ? 'number' : 'text'} placeholder={bulkField ? `Enter ${bulkField}` : ''} value={bulkValue} onChange={e => setBulkValue(e.target.value)} disabled={!bulkField}
                className="bg-fp-card text-fp-text border border-fp-border rounded px-2 py-1.5 text-xs focus:outline-none focus:border-fp-brand disabled:opacity-40" />
            )}
            <button onClick={onApplyBulkEdit} disabled={!bulkField || !bulkValue || bulkSelectedLines.size === 0}
              className="bg-fp-brand text-white px-3 py-1.5 rounded text-xs font-semibold hover:bg-[#b5571f] transition-colors disabled:opacity-40 whitespace-nowrap">
              Apply to Selected
            </button>
            {bulkSelectedLines.size > 0 && (
              <button onClick={() => setBulkSelectedLines(new Set())} className="text-fp-muted hover:text-fp-text text-xs transition-colors">Clear</button>
            )}
          </div>
                      ...
                      {sectionLines.map(line => {
                        ...
                        return (
                          <tr key={i} className={`border-b border-fp-border/30 ${bulkSelectedLines.has(rowKey) ? 'bg-[#C8622A]/5' : ''}`}>
                            ...
                            <td className="pr-2 py-1">
                              <select value={line.unit || 'ea'} onChange={e => onUpdateEditLine(i, 'unit', e.target.value)} className="bg-fp-inset text-fp-text border border-fp-border rounded px-2 py-1 text-xs focus:outline-none focus:border-fp-brand">
                                {['ea','ft','lot','hr','box','roll'].map(u => <option key={u}>{u}</option>)}
                              </select>
                            </td>
                            <td className="pr-2 py-1">
                              <select value={line.category || ''} onChange={e => onUpdateEditLine(i, 'category', e.target.value)} className="bg-fp-inset text-fp-text border border-fp-border rounded px-2 py-1 text-xs focus:outline-none focus:border-fp-brand">
                                <option value="">Category</option>
                                {categories.map(c => <option key={c}>{c}</option>)}
                              </select>
                            </td>
                            <td className="pr-2 py-1 min-w-[120px]">
                              ...
                              {line.vendor && !vendors.some(v => v.vendor_name === line.vendor) && (
                                <input type="text" placeholder="Vendor name" value={line.vendor === '__custom__' ? '' : line.vendor}
                                  ...
                                  className="w-full mt-1 bg-fp-inset text-fp-text border border-[#C8622A]/40 rounded px-2 py-1 text-xs focus:outline-none focus:border-fp-brand" />
                              )}
                            </td>
                            <td className="pr-2 py-1">
            ...
            return (
              <div className="space-y-6">
                <div>
                  {editSections.length > 0 && (
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-fp-muted text-xs font-semibold uppercase tracking-wide">General</span>
                      <span className="text-fp-muted text-xs">— items not assigned to a section</span>
                    </div>
                  )}
                ...
                {editSections.map(section => (
                  <div key={section.id} className="border border-fp-border rounded-xl overflow-hidden">
                    <div className="flex items-center gap-3 px-4 py-3 bg-fp-inset border-b border-fp-border">
                      ...
                      <label className="flex items-center gap-2 cursor-pointer">
                        <span className="text-fp-muted text-xs">Include Labor</span>
                        <button onClick={() => onUpdateSection(section.id, 'include_labor', !section.include_labor)}
                          className={`w-9 h-5 rounded-full transition-colors relative ${section.include_labor ? 'bg-[#C8622A]' : 'bg-fp-inset'}`}>
                          <span className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-all ${section.include_labor ? 'left-4' : 'left-0.5'}`} />
                        </button>
                      </label>
                      <button onClick={() => onDeleteSection(section.id)} className="text-fp-muted hover:text-red-400 text-xs transition-colors ml-2">✕ Remove</button>
                    </div>
                    <div className="p-4">
                            ...
                            <tbody>
                              {(section.labor_items || []).map((item, idx) => (
                                <tr key={idx} className="border-b border-fp-border/30">
                                  ...
                                  <td className="pr-2 py-1">
                                    <select value={item.unit || 'hr'} onChange={e => onUpdateSectionLabor(section.id, idx, 'unit', e.target.value)} className="bg-fp-inset text-fp-text border border-fp-border rounded px-2 py-1 text-xs focus:outline-none focus:border-fp-brand">
                      ...
                      {(() => {
                        ...
                        return (
                          <div className="flex justify-between items-center mt-3 pt-3 border-t border-fp-border text-xs">
                            <div className="flex gap-4">
                              <span className="text-fp-muted">Materials: <span className="text-fp-text font-semibold">${secMat.toLocaleString('en-US',{minimumFractionDigits:2})}</span></span>
                              {section.include_labor && secLab > 0 && <span className="text-fp-muted">Labor: <span className="text-fp-text font-semibold">${secLab.toLocaleString('en-US',{minimumFractionDigits:2})}</span></span>}
                              <span className="text-fp-muted">Margin: <span className="text-[#C8622A] font-semibold">{secMargin}%</span></span>
                            </div>
                            <span className="text-fp-muted">Section Total: <span className="text-fp-text font-bold">${secTotal.toLocaleString('en-US',{minimumFractionDigits:2})}</span></span>
                          </div>
                        )
                      })()}
                    </div>
          ...
          <div className="mt-8">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {laborItems.map((item, index) => (
                  <tr key={index} className="border-b border-fp-border/30">
                    ...
                    <td className="pr-2 py-1"><input type="number" placeholder="0" value={item.quantity || ''} onChange={e => onUpdateLabor(index, 'quantity', e.target.value)} className="w-full bg-fp-inset text-fp-text border border-fp-border rounded px-2 py-1 text-xs focus:outline-none focus:border-fp-brand" /></td>
                    <td className="pr-2 py-1">
                      <select value={item.unit || 'hr'} onChange={e => onUpdateLabor(index, 'unit', e.target.value)} className="bg-fp-inset text-fp-text border border-fp-border rounded px-2 py-1 text-xs focus:outline-none focus:border-fp-brand">
                        {['hr', 'day', 'lot'].map(u => <option key={u}>{u}</option>)}
                      </select>
                    </td>
                    ...
                    <td className="py-1">
                      <div className="flex items-center gap-2">
                        <button onClick={() => setLaborItems(prev => prev.filter((_, i) => i !== index))} className="text-fp-muted hover:text-red-400 text-xs">✕</button>
                        {editSections.length > 0 && (
                          <select
                            value=""
                            onChange={e => {
                            ...
                            className="bg-fp-card text-fp-muted hover:text-fp-text border border-fp-border rounded px-2 py-0.5 text-xs focus:outline-none focus:border-fp-brand cursor-pointer"
                          >
                            <option value="">→ Section</option>
              ...
              <tfoot>
                <tr>
                  <td colSpan="5" className="text-fp-muted pt-3 text-right font-semibold text-xs">Total Labor</td>
            ...
            <button onClick={() => setLaborItems(prev => [...prev, { role: '', quantity: '', unit: 'hr', your_cost: '', markup: defaultMarkup, customer_price: 0 }])} className="mt-4 text-[#C8622A] hover:text-fp-text text-sm transition-colors">+ Add Labor</button>
          </div>
          ...
          <div className="mt-6 border-t border-fp-border pt-4 space-y-2">
            {(() => {
              ...
              return (
                <>
                  <div className="flex justify-between text-sm"><span className="text-fp-muted">Materials</span><span className="text-fp-text">${liveBOMTotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></div>
                  <div className="flex justify-between text-sm"><span className="text-fp-muted">Labor</span><span className="text-fp-text">${liveLaborTotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></div>
                  {liveTaxRate > 0 && <div className="flex justify-between text-sm"><span className="text-fp-muted">Tax ({liveTaxRate}% on materials)</span><span className="text-fp-text">${liveTaxAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></div>}
                  <div className="flex justify-between text-base font-bold border-t border-fp-border pt-2"><span className="text-fp-text">Grand Total</span><span className="text-[#C8622A]">${liveGrandTotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></div>
                  <div className="flex justify-between text-sm"><span className="text-fp-muted">Gross Margin</span><span className="text-[#C8622A] font-semibold">{liveMargin}%</span></div>
                </>
              )
            })()}
          </div>
