    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-4">
      <div className="bg-fp-card rounded-2xl p-6 w-full max-w-md">
        ...
        <div className="space-y-4">
          <div>
            <label className="text-fp-muted text-xs mb-1 block">Vendor Email <span className="font-normal text-fp-muted">(optional — for your records)</span></label>
            ...
            <div className="space-y-1 max-h-40 overflow-y-auto">
              {selectedItems.map(item => (
                <div key={item.id} className="flex justify-between text-xs py-0.5">
                  ...
                  <span className="text-fp-muted">{item.vendor ? `${item.vendor} · ` : ''}Qty: {item.quantity} {item.unit}</span>
                </div>
              ))}
            </div>
          </div>
