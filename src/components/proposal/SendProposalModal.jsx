      ...
      <div className="bg-[#1a2d45] rounded-2xl p-6 w-full max-w-lg">
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button onClick={onSend} disabled={sendingProposal || !sendForm.subject || !sendForm.message} className="flex-1 bg-green-600 text-white py-2 rounded-lg text-sm font-semibold hover:bg-green-700 transition-colors disabled:opacity-50">{sendingProposal ? 'Sending...' : 'Send Proposal →'}</button>
          </div>
        </div>
      </div>
    </div>
