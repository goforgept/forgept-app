    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-4">
      <div className="bg-fp-card rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-5">
          <h3 className="text-fp-text font-bold text-lg">📡 {editingAgreementIdx !== null ? 'Edit' : 'Add'} Monitoring Contract</h3>
        ...
        <div className="space-y-4">
          {allMonitoringTemplates().length > 0 && (
            <div>
              ...
              }} defaultValue="" className="w-full bg-fp-inset text-fp-text border border-fp-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-fp-brand">
              ...
              <p className="text-fp-muted text-xs mt-1">All fields remain editable after selecting.</p>
            </div>
          )}
          <div>
            <label className="text-fp-muted text-xs mb-1 block">Contract Name</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              <input type="text" value={editMonitoringForm.monitored_systems || ''} onChange={e => setEditMonitoringForm(p => ({ ...p, monitored_systems: e.target.value }))}
                placeholder="e.g. Cameras, Access Control, Alarm" className="w-full bg-fp-inset text-fp-text border border-fp-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-fp-brand" />
            </div>
            <div>
            ...
            <div className="col-span-2 flex items-center gap-3">
              <input type="checkbox" id="mon-autorenew" checked={!!editMonitoringForm.auto_renew} onChange={e => setEditMonitoringForm(p => ({ ...p, auto_renew: e.target.checked }))} className="w-4 h-4 accent-fp-brand" />
              <label htmlFor="mon-autorenew" className="text-fp-muted text-sm">Auto-renew when term ends</label>
            </div>
          </div>
          <div>
