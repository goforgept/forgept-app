        ...
        <div className="space-y-5">
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              <p className="text-fp-muted text-xs font-semibold uppercase tracking-wide">Materials</p>
              <button onClick={() => setCoForm(p => ({ ...p, line_items: [...(p.line_items || []), { id: crypto.randomUUID(), item_name: '', quantity: 1, unit: 'ea', your_cost_unit: '', markup_percent: 35, customer_price_unit: '' }] }))}
                className="text-[#C8622A] text-xs hover:text-fp-text transition-colors">+ Add Material</button>
            </div>
            {(coForm.line_items || []).length === 0
              ? <p className="text-fp-muted text-xs italic">No materials added yet.</p>
              : (
                      ...
                      {(coForm.line_items || []).map((line, i) => {
                        ...
                        return (
                          <tr key={line.id} className="border-b border-fp-border/30">
                            ...
                            <td className="pr-2 py-1 text-fp-text font-medium">${lineTotal.toLocaleString('en-US',{minimumFractionDigits:2})}</td>
                            <td className="py-1"><button onClick={() => setCoForm(p => ({...p, line_items: (p.line_items||[]).filter((_,idx)=>idx!==i)}))} className="text-fp-muted hover:text-red-400">✕</button></td>
                          </tr>
                        )
                      })}
                    </tbody>
                    <tfoot>
                      <tr><td colSpan="6" className="text-fp-muted text-right pt-2 pr-2 font-semibold">Materials Total</td><td className="text-[#C8622A] font-bold pt-2">${matTotal.toLocaleString('en-US',{minimumFractionDigits:2})}</td><td></td></tr>
                    </tfoot>
                  </table>
                </div>
              )
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              ...
              <button onClick={() => setCoForm(p => ({ ...p, labor_items: [...(p.labor_items || []), { id: crypto.randomUUID(), role: '', quantity: '', unit: 'hr', your_cost: '', markup: 35, customer_price: '' }] }))}
                className="text-[#C8622A] text-xs hover:text-fp-text transition-colors">+ Add Labor</button>
            </div>
            {(coForm.labor_items || []).length === 0
              ? <p className="text-fp-muted text-xs italic">No labor added yet.</p>
              : (
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead>
                    ...
                    <tbody>
                      {(coForm.labor_items || []).map((labor, i) => (
                        <tr key={labor.id} className="border-b border-fp-border/30">
                          ...
                          <td className="pr-2 py-1 text-fp-text font-medium">${(parseFloat(labor.customer_price)||0).toLocaleString('en-US',{minimumFractionDigits:2})}</td>
                          <td className="py-1"><button onClick={() => setCoForm(p => ({...p,labor_items:(p.labor_items||[]).filter((_,idx)=>idx!==i)}))} className="text-fp-muted hover:text-red-400">✕</button></td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot>
                      <tr><td colSpan="5" className="text-fp-muted text-right pt-2 pr-2 font-semibold">Labor Total</td><td className="text-[#C8622A] font-bold pt-2">${labTotal.toLocaleString('en-US',{minimumFractionDigits:2})}</td><td></td></tr>
                    </tfoot>
                  </table>
                </div>
              )
