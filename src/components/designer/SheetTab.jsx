...
export default function SheetTab({ sheet, isActive, onSelect, onRename, onDelete }) {
  ...
  }[sheet.status] || 'bg-fp-inset text-fp-muted'
    ...
    <div onClick={onSelect}
      className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium cursor-pointer transition-colors whitespace-nowrap group ${
        isActive
          ...
          : 'text-fp-muted hover:bg-fp-card border border-transparent'
      }`}>
      {editing ? (
        <input autoFocus value={name}
          ...
          className="w-20 text-xs border-b border-[#C8622A] outline-none bg-transparent text-fp-text" />
      ) : (
        <span onDoubleClick={(e) => { e.stopPropagation(); setEditing(true) }}>{sheet.name}</span>
      )}
      {/* Rename icon */}
      <button onClick={(e) => { e.stopPropagation(); setEditing(true) }}
        className="opacity-0 group-hover:opacity-100 transition-opacity text-fp-muted hover:text-fp-text"
        title="Rename">
